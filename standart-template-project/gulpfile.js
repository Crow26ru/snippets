...
gulp.task('scripts-modules', function () {
  /**
  * По мере добавления новых файлов JavaScript, добавляейте в массив,
  * который передается в gulp.src() пути в том порядке, в котором они должны подключаться.
  * Самым первым файлом всегда должен быть util.js, так как это файл общих ресурсов.
  */
  return gulp.src([
